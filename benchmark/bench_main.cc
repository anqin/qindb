...
int main(int argc, char* argv[]) {
    ...
    int ret = 0;
    qindexdb::Status status;
    qindexdb::Options options;
    options.base_dir = "my_db";
    options.file_size = (8 << 10);
    qindexdb::Database database(options);
    std::string cmd = argv[1];
    if (cmd == "fill") {
        ret = qindexdb::FillOp(&database, argc, argv, &status);
    } else if (cmd == "fetch") {
        ret = qindexdb::FetchOp(&database, argc, argv, &status);
    } else if (cmd == "remove") {
        ret = qindexdb::RemoveOp(&database, argc, argv, &status);
    } else if (cmd == "put" || cmd == "get" || cmd == "delete") {
        ret = qindexdb::BaseOp(&database, argc, argv, &status);
    } else {
        Usage(argv[0]);
    }
    return ret;
